...
def main(exp, target_idx, num_views):
    ...
    # print(f"images_before: {len(images_before)}, images_after: {len(images_after)}")
    ...
    print("ID Sim_avg: {:.4f} ± {:.4f}".format(np.mean(idsims_avg), np.std(idsims_avg)))
    print("ID Sim: {:.4f} ± {:.4f}".format(np.mean(idsims), np.std(idsims)))
    print("ID Sim_others: {:.4f} ± {:.4f}".format(np.mean(idsims_others), np.std(idsims_others)))
